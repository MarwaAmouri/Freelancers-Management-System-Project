...
const actions = {
 ...
 const clientReducer = (state, action) => {
    switch (action.type) {
      case actions.SIGN_UP:
        ...
        case actions.UPDATE_BALANCE:
          ...
          const updatedClientUsers = state.registeredClientUsers.map((user) => {
          ...
          // localStorage.setItem('registeredClientUsers', JSON.stringify(updatedClientUsers));
          localStorage.setItem('users', JSON.stringify(updatedUsers));

          return {
            ...state,
            registeredClientUsers: updatedClientUsers,
  ...
  function ClientContextProvider({ children }) {
    // useReducer initialization function:
    ...
    useEffect(() => {
      ...
      // To ensure that the users and registeredClientUsers are not empty and not undefined type(when start the application at first time).
      if (Array.isArray(parsedRegisteredUsers) && state.registeredClientUsers.length > 0) {  
        ...
        if (parsedRegisteredUsers === null) {
          localStorage.removeItem('registeredClientUsers'); // Remove the data if it's null, cleanup step to ensure that there's no residual data from a previous session when the application starts.
        }
      }
      ...
      if (state.currentClientUser) {
        ...
        localStorage.removeItem('currentClientUser'); // Remove the data if it's null when user log out 
      }
